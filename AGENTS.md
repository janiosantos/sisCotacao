...
- **VM 10.189.14.9** (Ubuntu `dsktop`): stack Docker em `~/Projetos/ecommerce_scraper`, frontend `http://10.189.14.9:8080`. Acesso SSH: `jpsantos@10.189.14.9` (chave pública do notebook; sudo com senha armazenada fora do repositório).
...
**Ferramentas na VM**: Docker 29 + Compose 2.40, Git 2.53, Python 3.14, **opencode** (v1.18.23 em `~/.opencode/bin`, PATH no `~/.bashrc`). Instalar extras via SSH com autenticação por chave e procedimento seguro fora do repositório.
...
### Ambiente / infra
9. `frontend/tests/` — ✅ **esqueleto entregue (v2.15.0)**: vitest + `vitest.config.ts` + `tests/format.test.ts`; script `npm test`. **Hoje: 34 testes** (ApiError/`mensagemErro`, cache/contrato runtime do client, Table/Badge, `parseBusca` do PDV e foco do Modal). Dívida residual: testes E2E de fluxos críticos.
