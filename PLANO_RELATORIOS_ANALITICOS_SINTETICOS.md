...
- **P1.4/P1.5/P1.6:** entregues endpoints e telas analíticas de vendas, compras e posição de estoque, com agrupamentos, nomes, CMV/margem, recebimento parcial, saldo zero, situação operacional, paginação, impressão e exportação. A tela agora possui pesquisa, paginação, concentração de fornecedores, links para pedido impresso e visão de necessidade de compra reutilizando o motor de reposição. Ainda falta drill-down interativo para documento dentro da própria SPA e a integração visual completa com o módulo ABC histórico.
- **P2.1:** entregue o contrato `financeiro.analitico` com aging por faixas não sobrepostas, títulos parciais, fluxo de caixa, DRE gerencial por competência operacional e despesas pagas por categoria; possui endpoint, RBAC financeiro, exportação e impressão. Ainda falta comparação de períodos, filtros visuais de corte/tipo, detalhamento de conciliação e revisão contábil antes de qualquer uso como DRE fiscal.
- **P2 restante:** ABC/XYZ histórica como relatório gerencial dedicado, rentabilidade por obra/consumo, relatórios fiscais, RFM/segmentação avançada, visões salvas, agendamento, exportação assíncrona e E2E ainda não foram implementados nesta etapa.
