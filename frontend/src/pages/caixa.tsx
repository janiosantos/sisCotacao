    ...
    <div className="flex min-h-[560px] flex-col">
      ...
      <header className="flex flex-shrink-0 items-center justify-between gap-3 border-b border-gray-300 bg-[#e4e4e4] px-4 py-1.5 text-sm text-gray-800">
        ...
        <div className="flex min-w-0 flex-1 items-center gap-2">
          <span>Pedido:</span>
          <span className="max-w-md truncate rounded border border-gray-400 bg-white px-2 py-0.5 text-sm font-medium text-gray-800">
            {selecionado ? `${selecionado.numero} · ${selecionado.cliente || "—"}` : "Nenhum pedido selecionado"}
          </span>
          <span className="text-[10px] text-gray-500">F8 localizar</span>
        </div>
        <div>Vendedor: {selecionado?.usuario_nome ?? "—"}</div>
      ...
      <main className="flex min-h-[480px] flex-1 flex-col gap-3 overflow-hidden bg-[#6a84a6] p-4">
        ...
        <div className="flex flex-shrink-0 items-center justify-between rounded-xl bg-white p-3 shadow-md">
          <div>
            <span className="text-sm font-bold text-gray-800">Pedido</span>
            <div className="mt-0.5 text-xs text-gray-500">
              Selecione uma venda faturada para receber o pagamento.
            </div>
          </div>
        ...
        <div className="flex min-h-0 flex-1 gap-4">
          ...
          <div className="flex w-6/12 min-h-0 flex-col overflow-hidden rounded-[2.5rem] bg-white p-4 font-mono text-sm shadow-md">
            <div className="grid grid-cols-[1fr_60px_84px_84px] gap-1 text-[11px] uppercase text-gray-500">
              <span>Descrição</span>
            ...
            <div className="min-h-0 flex-1 overflow-auto">
              {!selecionado ? (
                ...
                selecionado.itens.map((it, i) => (
                  <div
                    key={i}
                    className="grid grid-cols-[1fr_60px_84px_84px] items-center gap-1 border-b border-gray-100 py-1.5"
                  >
        ...
        <div className="flex h-24 flex-shrink-0 gap-4">
          <div className="flex w-4/12 items-center justify-center rounded-xl bg-white shadow-md">
            <span className="text-5xl font-bold tracking-widest text-black">{selecionado ? selecionado.numero : "CAIXA"}</span>
          </div>
      ...
      <footer className="flex flex-shrink-0 flex-wrap items-center gap-2 border-t border-gray-400 bg-[#f0f0f0] px-4 py-2">
        ...
        <Button size="sm" variant="ghost" onClick={() => setModalMovimento("suprimento")}>
          Reforço (R)
        </Button>
...
function ModalLocalizarPedido({ onClose, onSelecionar }: { onClose: () => void; onSelecionar: (id: number) => void }) {
  ...
  return (
    <SearchModal
      open
      title="Localizar pedido (faturado)"
      columns={[
