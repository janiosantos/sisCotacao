// page_historico.js — histórico e evolução de preços por produto.
const PageHistorico = (() => {
  ...
  async function render($app) {
    ...
    const $busca = $app.querySelector("#fBusca");
    const $sug = $app.querySelector("#fSugestoes");
    $busca.addEventListener("input", () => {
      ...
      const matches = codigos.filter((c) => (c.sku + " " + c.name).toLowerCase().includes(q)).slice(0, 20);
      $sug.innerHTML = matches.map((c) => `
        <div class="sug-item" data-id="${c.id}" style="padding:8px 10px;font-size:12.5px;cursor:pointer;border-bottom:1px solid var(--line);">
          <span style="font-family:var(--font-mono);color:var(--steel);">${UI.escapeHtml(c.sku || "#" + c.id)}</span> — ${UI.escapeHtml(c.name)}
      ...
      $sug.querySelectorAll("[data-id]").forEach((el) => {
        ...
        el.addEventListener("click", () => {
          ...
          $sug.style.display = "none";
          carregarProduto($app, Number(el.dataset.id));
        });
      });
    });
    document.addEventListener("click", (e) => {
      if (!$sug.contains(e.target) && e.target !== $busca) $sug.style.display = "none";
    });
  }

  async function carregarProduto($app, produtoId) {
        ...
        <table class="data-table">
          ...
          <tbody>
            ${rows.slice().reverse().map((r) => `
              <tr>
                <td>${UI.fmtDateTime(r.registrado_em)}</td>
                <td>${UI.escapeHtml(r.fornecedor_nome)}</td>
                <td><a href="#/cotacoes/${r.cotacao_id}" style="font-family:var(--font-mono);">${r.cotacao_numero}</a></td>
                <td style="font-family:var(--font-mono);">${UI.fmtMoney(r.preco_unitario)}</td>
  ...
  function buildChart(porFornecedor, cores) {
    const W = 760, H = 260, PAD_L = 55, PAD_B = 30, PAD_T = 16, PAD_R = 16;
    const allPontos = Object.values(porFornecedor).flat();
    const precos = allPontos.map((p) => p.preco_unitario);
    let min = Math.min(...precos), max = Math.max(...precos);
    ...
    const yFor = (preco) => PAD_T + (1 - (preco - min) / (max - min)) * (H - PAD_T - PAD_B);
    ...
    for (const [nome, pontos] of Object.entries(porFornecedor)) {
      ...
      for (const p of sorted) {
        linesHtml += `<circle cx="${xFor(p.registrado_em)}" cy="${yFor(p.preco_unitario)}" r="3.5" fill="${cores[nome]}"><title>${UI.escapeHtml(nome)}: ${UI.fmtMoney(p.preco_unitario)}</title></circle>`;
      }
    }

    const xLabels = datas.length <= 6 ? datas : [datas[0], datas[Math.floor(datas.length / 2)], datas[datas.length - 1]];
    const xLabelsHtml = xLabels.map((d) => `<text x="${xFor(d)}" y="${H - 8}" text-anchor="middle" font-size="10" font-family="var(--font-mono)" fill="var(--ink-faint)">${UI.fmtDate(d)}</text>`).join("");
