...
def test_saldo_inicial_zero(produto_id):
    # cria a linha de saldo (0) com um movimento neutro
    estoque_repo.movimentar(DEPOSITO, produto_id, "entrada", 0, documento="TST-0")
    saldo = estoque_repo.saldo(deposito_id=DEPOSITO, produto_id=produto_id)
    assert len(saldo) == 1
...
def test_movimentar_entrada(produto_id):
    res = estoque_repo.movimentar(DEPOSITO, produto_id, "entrada", 10, documento="TST-1")
    assert res["saldo_anterior"] == 0
    assert res["saldo_posterior"] == 10
    assert _saldo(produto_id) == 10
    movs = estoque_repo.movimentos(deposito_id=DEPOSITO, produto_id=produto_id)
    assert len(movs) == 1
...
def test_movimentar_saida_nao_negativa(produto_id):
    estoque_repo.movimentar(DEPOSITO, produto_id, "entrada", 5)
    res = estoque_repo.movimentar(DEPOSITO, produto_id, "saida", 3)
    assert res["saldo_posterior"] == 2
    res2 = estoque_repo.movimentar(DEPOSITO, produto_id, "saida", 99)
    assert res2["saldo_posterior"] == 0


def test_movimentar_ajuste(produto_id):
    estoque_repo.movimentar(DEPOSITO, produto_id, "entrada", 5)
    res = estoque_repo.movimentar(DEPOSITO, produto_id, "ajuste", 7)
    assert res["saldo_posterior"] == 7
    res2 = estoque_repo.movimentar(DEPOSITO, produto_id, "ajuste", 0)
    assert res2["saldo_posterior"] == 0


def test_transferir_entre_depositos(produto_id, deposito_2):
    estoque_repo.movimentar(DEPOSITO, produto_id, "entrada", 8)
    res = estoque_repo.transferir(DEPOSITO, deposito_2, produto_id, 3)
    assert res["ok"]
    assert _saldo(produto_id) == 5
    with system_conn() as conn:
        destino = conn.execute(
            "SELECT quantidade FROM estoque_saldo WHERE deposito_id=? AND produto_id=?",
            (deposito_2, produto_id),
        ).fetchone()["quantidade"]
    assert destino == 3


def test_lotes_create_list(produto_id):
    lote_id = lote_repo.create(
        DEPOSITO, produto_id, "LOT-1", quantidade=10,
    ...
    lotes = lote_repo.list(deposito_id=DEPOSITO, produto_id=produto_id)
    assert len(lotes) == 1
