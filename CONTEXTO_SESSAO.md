...
- **Hardening (Codex)**: RBAC deny-by-default (sem perfil → 403), `MAX_CONTENT_LENGTH`, `safe_http` (SSRF), rate limit de login (PostgreSQL, 5/300s), segredos de pagamento não vazam, `deploy.yml` passa `CATALOG_SECRET`/`POSTGRES_USER`/`POSTGRES_PASSWORD` (secrets do GitHub configurados). **236 testes** backend + **27 testes** frontend verdes.
...
- **Frontend (P6)**: **29 telas modularizadas, 93 módulos**; `ApiError`/`mensagemErro`, cache curto de GET com invalidação, validação runtime mínima de contratos críticos e Error Boundary; 29 testes; mojibake 0 em todo o frontend.
...
- [ ] Ampliar cobertura de testes frontend (vitest — hoje 29) e E2E de fluxos críticos.
